...
**Step 3:** Fix any missing transitions with `adventure_create_transition`. This is a **single bidirectional call** — pass `areaA`, `aX`, `aY`, `areaB`, `bX`, `bY`, and a unique `tag` (max 11 chars). It places a blue light portal + landing waypoint in both areas at the correct positions. Do NOT call twice.
